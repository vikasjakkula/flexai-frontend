...
TEST_LIST_URL = f"{BASE_URL}/exams/eamcet-previous-question-papers-online"
OUTPUT_DIR = "ap_scraped_data_2"
...
async def extract_and_upload_sprites(page, test_slug: str, progress: dict):
    ...
    - ts_12_may_2023_Shift1.css (capital Shift, no underscore)
    - ts_2022_july_18_s2.css (year first, full month)
    - ts_4_aug_2021_Shift_1.css (capital Shift with underscore)
    - ts_9_sept_2020_s1.css (sept abbreviation)
    - ts2019.css (year only)
    - ts2019a.css (year with letter suffix)
    - ts_2019_may_4_s2.css (year first format)
    ...
    try:
        ...
        year = None
        shift = "1"
        year_suffix = None  # For patterns like ts2019a.css
        
        # Month name mapping (full to various abbreviations)
        month_map = {
            'jan': 'jan', 'january': 'jan',
        ...
        for i, part in enumerate(slug_parts):
            if part.isdigit() and len(part) <= 2:  # day
                day_raw = part  # Keep original (e.g., "4" or "12")
                day = part.zfill(2)  # Zero-pad single digits (4 -> 04)
                if i + 2 < len(slug_parts):
                    month_name = slug_parts[i + 1]
                    month_full = month_name
                    month = month_map.get(month_name, month_name[:3])  # Get mapped abbrev or first 3 chars
                    year = slug_parts[i + 2]
                    break
        ...
        # Check for year suffix (like "code-a" for ts2019a.css)
        # Look for patterns like "2017-code-a" where code comes after year
        if not year:
            for i, part in enumerate(slug_parts):
                if part.isdigit() and len(part) == 4:  # 4-digit year
                    year = part
                    # Check if there's "code-X" after the year (e.g., 2017-code-a)
                    if i + 2 < len(slug_parts) and slug_parts[i + 1] == 'code':
                        code_part = slug_parts[i + 2]
                        if len(code_part) == 1 and code_part.isalpha():
                            year_suffix = code_part.lower()
                    break
        else:
            # Also check for code suffix after finding year
            for i, part in enumerate(slug_parts):
                if part == year:
                    # Check if there's "code-X" after the year (e.g., 2017-code-a)
                    if i + 2 < len(slug_parts) and slug_parts[i + 1] == 'code':
        ...
        # Pattern 1: Date-based patterns (day-month-year order)
        if day and month and year:
            # Normalize day (try both with and without zero padding for single digits)
            day_variants = [day]  # Always include zero-padded version
            if day_raw and day_raw != day:  # Also include original if different
                day_variants.append(day_raw)
            
            for day_var in day_variants:
                # Pattern 1a: ts_{day}_{month}_{year}_s{shift}.css (lowercase s)
                base_s = f"ts_{day_var}_{month}_{year}_s{shift}"
                url_patterns.append({
                    "name": f"ts_{day_var}_{month}_{year}_s{shift}",
                    "css": f"{cdn_base}/{base_s}.css",
                    "png_variants": [
                        f"{cdn_base}/{base_s}-{shift}.png",
                        f"{cdn_base}/{base_s}-1.png",
                        f"{cdn_base}/{base_s}-0.png",
                        f"{cdn_base}/{base_s}_0.png",
                        f"{cdn_base}/{base_s}.png"
                    ]
                })
                
                # Pattern 1b: ts_{day}_{month}_{year}_Shift{shift}.css (capital Shift, no underscore)
                base_Shift = f"ts_{day_var}_{month}_{year}_Shift{shift}"
                url_patterns.append({
                    "name": f"ts_{day_var}_{month}_{year}_Shift{shift}",
                ...
                # Pattern 1c: ts_{day}_{month}_{year}_Shift_{shift}.css (capital Shift with underscore)
                base_Shift_ = f"ts_{day_var}_{month}_{year}_Shift_{shift}"
                url_patterns.append({
                    "name": f"ts_{day_var}_{month}_{year}_Shift_{shift}",
                    "css": f"{cdn_base}/{base_Shift_}.css",
                    "png_variants": [
                        f"{cdn_base}/{base_Shift_}-{shift}.png",  # e.g., ts_4_aug_2021_Shift_1-1.png
                        f"{cdn_base}/{base_Shift_}-1.png",
                        f"{cdn_base}/{base_Shift_}-0.png",
                        f"{cdn_base}/{base_Shift_}_0.png",
                        f"{cdn_base}/{base_Shift_}.png"
                    ]
                })
                
                # Pattern 1d: Without shift (ts_{day}_{month}_{year}.css)
                base_no_shift = f"ts_{day_var}_{month}_{year}"
                url_patterns.append({
                    "name": f"ts_{day_var}_{month}_{year}",
                    "css": f"{cdn_base}/{base_no_shift}.css",
                    "png_variants": [
                        f"{cdn_base}/{base_no_shift}_0.png",
                        f"{cdn_base}/{base_no_shift}-0.png",
                        f"{cdn_base}/{base_no_shift}.png"
                    ]
                })
        
        # Pattern 2: Year-month-day order (ts_{year}_{month}_{day}_s{shift}.css)
        if day and month and year:
            day_var = day_raw if day_raw else day
            # Pattern 2a: ts_{year}_{month}_{day}_s{shift}.css (year-month-day order)
            base_ymd_s = f"ts_{year}_{month_full}_{day_var}_s{shift}"
            url_patterns.append({
                "name": f"ts_{year}_{month_full}_{day_var}_s{shift}",
                "css": f"{cdn_base}/{base_ymd_s}.css",
                "png_variants": [
                    f"{cdn_base}/{base_ymd_s}-1.png",  # Common pattern (e.g., ts_2022_july_18_s2-1.png)
                    f"{cdn_base}/{base_ymd_s}-{shift}.png",
                    f"{cdn_base}/{base_ymd_s}-0.png",
                    f"{cdn_base}/{base_ymd_s}_0.png",
                    f"{cdn_base}/{base_ymd_s}.png"
                ]
            })
            
            # Pattern 2b: ts_{year}_{month}_{day}_s{shift}.css with abbreviated month
            if month != month_full:
                base_ymd_s_abbrev = f"ts_{year}_{month}_{day_var}_s{shift}"
                url_patterns.append({
                    "name": f"ts_{year}_{month}_{day_var}_s{shift}",
                    "css": f"{cdn_base}/{base_ymd_s_abbrev}.css",
                    "png_variants": [
                        f"{cdn_base}/{base_ymd_s_abbrev}-1.png",
                        f"{cdn_base}/{base_ymd_s_abbrev}-{shift}.png",
                        f"{cdn_base}/{base_ymd_s_abbrev}-0.png",
                        f"{cdn_base}/{base_ymd_s_abbrev}_0.png",
                        f"{cdn_base}/{base_ymd_s_abbrev}.png"
                    ]
                })
            
            # Pattern 2c: ts_{year}_{month}_{day}_s{shift}.css (with "sept" variant)
            if month_full in ['sep', 'sept', 'september']:
                base_sept = f"ts_{year}_sept_{day_var}_s{shift}"
                url_patterns.append({
                    "name": f"ts_{year}_sept_{day_var}_s{shift}",
                    "css": f"{cdn_base}/{base_sept}.css",
                    "png_variants": [
                        f"{cdn_base}/{base_sept}.png",
                        f"{cdn_base}/{base_sept}_0.png",
                        f"{cdn_base}/{base_sept}-0.png"
                    ]
                })
        
        # Pattern 3: ts_{year}_{month}_{day}_s{shift}.css (year-month-day with abbreviated month, if different)
        # This is already covered in Pattern 2b, but we also need ts_2019_may_4_s2.css format
        if day and month and year:
            day_var = day_raw if day_raw else day
            # Pattern 3a: ts_{year}_{month}_{day}_s{shift}.css (year first, abbreviated month)
            # Check if this pattern is different from what we already added
            existing_pattern_names = [p['name'] for p in url_patterns]
            pattern_name = f"ts_{year}_{month}_{day_var}_s{shift}"
            if pattern_name not in existing_pattern_names and month != month_full:
                base_ymd_s_abbrev2 = f"ts_{year}_{month}_{day_var}_s{shift}"
                url_patterns.append({
                    "name": pattern_name,
                    "css": f"{cdn_base}/{base_ymd_s_abbrev2}.css",
                    "png_variants": [
                        f"{cdn_base}/{base_ymd_s_abbrev2}-1.png",
                        f"{cdn_base}/{base_ymd_s_abbrev2}-{shift}.png",
                        f"{cdn_base}/{base_ymd_s_abbrev2}-0.png",
                        f"{cdn_base}/{base_ymd_s_abbrev2}_0.png",
                        f"{cdn_base}/{base_ymd_s_abbrev2}.png"
                    ]
                })
        
        # Pattern 4: Year-only patterns
        if year and len(year) == 4:
            # Pattern 4a: ts{year}.css
            url_patterns.append({
                "name": f"ts{year}",
                "css": f"{cdn_base}/ts{year}.css",
                "png_variants": [
                    f"{cdn_base}/ts{year}.png"
                ]
            })
            
            # Pattern 4b: ts{year}{suffix}.css (e.g., ts2019a.css) - only if suffix found in slug
            if year_suffix:
                url_patterns.append({
                    "name": f"ts{year}{year_suffix}",
                    "css": f"{cdn_base}/ts{year}{year_suffix}.css",
                    "png_variants": [
                        f"{cdn_base}/ts{year}{year_suffix}.png"
                    ]
                })
        ...
        log_sprite(f"Extracted: day={day}, month={month} (full: {month_full}), year={year}, shift={shift}, suffix={year_suffix}")
...
async def main():
    """Main scraper - SEQUENTIAL (no parallel execution)"""
    print("=" * 60)
    print("TS EAMCET Question Scraper - SEQUENTIAL MODE")
    print("=" * 60)
    print()
    print("CONFIGURATION:")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not pending_tests:
                ...
                # Scrape tests ONE BY ONE (sequential)
                for i, test_info in enumerate(pending_tests):
                    log(f"\n[{i+1}/{len(pending_tests)}] Starting next test...")
