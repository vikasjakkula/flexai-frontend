...
High-performance parallel execution with safety features
"""
...
# Concurrency limits - adjust based on your system and server capacity
MAX_CONCURRENT_QUESTIONS = 10  # Number of questions to scrape in parallel
MAX_CONCURRENT_TESTS = 2  # Number of tests to process in parallel (lower to be safer)
MAX_BROWSER_PAGES = 15  # Maximum browser pages in pool
MIN_DELAY_BETWEEN_REQUESTS = 0.1  # Minimum delay between requests (seconds)
MAX_DELAY_BETWEEN_REQUESTS = 0.3  # Maximum delay for jitter
BATCH_SIZE = 50  # Process questions in batches to avoid memory issues
...
async def save_json_safe(filepath, data):
    """Save JSON file with async lock"""
    async with file_locks[filepath]:
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)


def save_json(filepath, data):
    """Save JSON file (synchronous, use with locks)"""
    with open(filepath, 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=2, ensure_ascii=False)


async def load_progress_safe():
    ...
    async with progress_lock:
        ...
        # Ensure the file exists by saving it if it doesn't
        if not os.path.exists(PROGRESS_FILE):
            save_json(PROGRESS_FILE, progress)
            log(f"Created progress.json file")
        return progress


async def save_progress_safe(progress):
    ...
    async with progress_lock:
        try:
            ...
            save_json(PROGRESS_FILE, progress)
            log(f"Progress saved: {len(progress.get('completed_tests', []))} completed tests, {len(progress.get('uploaded_sprite_sheets', []))} uploaded sprites")
        except Exception as e:
            log(f"ERROR saving progress: {str(e)}", "ERROR")
            import traceback
            traceback.print_exc()
...
async def load_test_data_safe(test_slug):
    """Load test data with lock"""
...
async def save_test_data_safe(test_slug, test_data):
    """Save test data with lock"""
    filepath = os.path.join(OUTPUT_DIR, f"{test_slug}.json")
    async with file_locks[filepath]:
...
async def question_exists_safe(test_slug, question_num):
    """Check if question already exists (thread-safe)"""
    test_data = await load_test_data_safe(test_slug)
    if test_data is None:
        return False
    return any(q['question_number'] == question_num for q in test_data.get('questions', []))


async def add_question_safe(test_slug, question_data):
    """Add question to test data (thread-safe)"""
    filepath = os.path.join(OUTPUT_DIR, f"{test_slug}.json")
...
async def extract_and_upload_sprites(page, test_slug: str, progress: dict):
    ...
    # Check if already uploaded for this test (thread-safe)
    async with progress_lock:
        progress_current = await load_progress_safe()
        if test_slug in progress_current.get('uploaded_sprite_sheets', []):
    ...
    try:
        ...
        for i, part in enumerate(slug_parts):
            if part.isdigit() and len(part) <= 2:  # day
                day_raw = part  # Keep original (e.g., "4" or "21")
                day = part  # Don't zero-pad for AP EAPCET patterns
        ...
        # Save CSS locally (ALWAYS save locally, even if upload fails)
        ...
        try:
            ...
            log_sprite(f"ERROR saving CSS locally: {str(e)}", "ERROR")
            # Continue anyway - try to upload
        
        # Save PNG locally (ALWAYS save locally, even if upload fails)
        png_filename = f"{sprite_sheet_id}.png"  # tg_eamcet_2_may_2025_shift_1_paper.png
        ...
        try:
            ...
            log_sprite(f"ERROR saving PNG locally: {str(e)}", "ERROR")
            # Continue anyway - try to upload
        ...
        # Mark as uploaded in progress (thread-safe)
        async with progress_lock:
            progress_current = await load_progress_safe()
            if 'uploaded_sprite_sheets' not in progress_current:
                progress_current['uploaded_sprite_sheets'] = []
            if test_slug not in progress_current['uploaded_sprite_sheets']:
                progress_current['uploaded_sprite_sheets'].append(test_slug)
                await save_progress_safe(progress_current)
                log_sprite(f"Progress updated: {test_slug} marked as uploaded")
...
class BrowserPagePool:
    """Manages a pool of browser pages for parallel scraping"""
    
    def __init__(self, browser, max_pages=MAX_BROWSER_PAGES):
        self.browser = browser
        self.max_pages = max_pages
        self.pages = asyncio.Queue(maxsize=max_pages)
        self.context = None
        self.created_pages = []
    ...
    async def initialize(self):
        ...
        log(f"Initialized browser page pool with {self.max_pages} pages")
...
async def scrape_single_question(page, test_url, question_num, semaphore):
    """
    Scrape a single question - PARALLEL VERSION
    ...
    async with semaphore:
        ...
        for retry in range(MAX_RETRIES):
            try:
                ...
                await asyncio.sleep(0.1)  # Small delay after navigation
                    ...
                    if not solution_html or len(solution_html.strip()) < 50:
                        try:
                            ...
                            for div in solution_divs:
                                ...
                                if div_html and len(div_html.strip()) > 50:
...
async def scrape_question_with_pool(page_pool, test_url, question_num, semaphore, test_slug, stats):
    """
    Scrape a question using page pool and handle errors safely
    """
    page = None
    try:
        page = await page_pool.get_page()
        result = await scrape_single_question(page, test_url, question_num, semaphore)
        
        if result:
            # Check if already exists (thread-safe)
            exists = await question_exists_safe(test_slug, question_num)
            if not exists:
                added = await add_question_safe(test_slug, result)
                if added:
                    stats['success'] += 1
                    log(f"  ✓ Q{question_num} saved (correct: {result['correct_option']})")
                else:
                    stats['skipped'] += 1
            else:
                stats['skipped'] += 1
            ...
            stats['failed'] += 1
            log(f"  ✗ Q{question_num} failed to scrape", "WARN")
    
    except Exception as e:
        stats['failed'] += 1
        log(f"  ✗ Q{question_num} error: {str(e)[:50]}", "ERROR")
...
async def scrape_test_parallel(page_pool, test_info, progress, question_semaphore):
    """
    Scrape a single test - PARALLEL VERSION
    Scrapes multiple questions in parallel with controlled concurrency
    """
    ...
    # Get a page for initial operations
    page = await page_pool.get_page()
    try:
        # Get total questions
        ...
        # Extract and upload sprites for this test (pass empty dict, function loads progress itself)
        await extract_and_upload_sprites(page, test_slug, {})
        ...
        if test_data is None:
            ...
            await save_test_data_safe(test_slug, test_data)
        else:
        ...
        log(f"Starting parallel scrape with {MAX_CONCURRENT_QUESTIONS} concurrent workers...")
        
        # Statistics tracking
        stats = {'success': 0, 'failed': 0, 'skipped': 0}
        
        # Process questions in batches to avoid memory issues
        for batch_start in range(0, len(missing_nums), BATCH_SIZE):
            batch = missing_nums[batch_start:batch_start + BATCH_SIZE]
            batch_num = (batch_start // BATCH_SIZE) + 1
            total_batches = (len(missing_nums) + BATCH_SIZE - 1) // BATCH_SIZE
            
            log(f"Processing batch {batch_num}/{total_batches} ({len(batch)} questions)...")
            
            # Create tasks for parallel execution
            tasks = [
                scrape_question_with_pool(page_pool, test_url, q_num, question_semaphore, test_slug, stats)
                for q_num in batch
            ]
            ...
            log(f"  Batch {batch_num} complete: {stats['success']} saved, {stats['failed']} failed, {stats['skipped']} skipped (Total: {completed}/{len(missing_nums)})")
        
        # Reload final data
        test_data = await load_test_data_safe(test_slug)
        log(f"Completed: {len(test_data['questions'])}/{total_questions} questions saved")
        log(f"Final stats: {stats['success']} new, {stats['failed']} failed, {stats['skipped']} skipped")
        
        # Mark test as completed (thread-safe)
        async with progress_lock:
            progress_current = await load_progress_safe()
            if 'completed_tests' not in progress_current:
                progress_current['completed_tests'] = []
            if test_slug not in progress_current['completed_tests']:
                progress_current['completed_tests'].append(test_slug)
                await save_progress_safe(progress_current)
                log(f"Progress updated: {test_slug} marked as completed")
        
        return test_data
    
    except Exception as e:
        log(f"ERROR in scrape_test_parallel for '{test_title}': {str(e)}", "ERROR")
        import traceback
        traceback.print_exc()
...
async def merge_all_data():
    ...
    await save_json_safe(ALL_DATA_FILE, all_data)
...
async def main():
    ...
    print(f"  Browser page pool size: {MAX_BROWSER_PAGES}")
    ...
    if not os.path.exists(PROGRESS_FILE):
        ...
        save_json(PROGRESS_FILE, initial_progress)
        log("Initialized progress.json file")
    
    progress = await load_progress_safe()
    ...
    async with async_playwright() as p:
        ...
        page_pool = BrowserPagePool(browser, max_pages=MAX_BROWSER_PAGES)
        await page_pool.initialize()
        
        # Semaphore for controlling question concurrency
        ...
        try:
            # Get a page for getting test links
            page = await page_pool.get_page()
            try:
                # Get all test links
            ...
            if not pending_tests:
                ...
                # Scrape tests with controlled concurrency
                async def scrape_test_with_semaphore(test_info):
                    async with test_semaphore:
                        try:
                            return await scrape_test_parallel(page_pool, test_info, progress, question_semaphore)
                        except Exception as e:
                            log(f"ERROR in scrape_test_with_semaphore for '{test_info['title']}': {str(e)}", "ERROR")
                            import traceback
                            traceback.print_exc()
                            return None
                
                # Process tests in parallel (with limit)
                log(f"Starting {len(pending_tests)} tests with max {MAX_CONCURRENT_TESTS} concurrent...")
                tasks = [scrape_test_with_semaphore(test_info) for test_info in pending_tests]
                results = await asyncio.gather(*tasks, return_exceptions=True)
                
                # Log results
                success_count = sum(1 for r in results if r is not None and not isinstance(r, Exception))
                error_count = sum(1 for r in results if isinstance(r, Exception))
                log(f"Test processing complete: {success_count} succeeded, {error_count} had errors")
                
                # Log any exceptions
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        test_info = pending_tests[i]
                        log(f"ERROR: Test '{test_info['title']}' failed: {str(result)}", "ERROR")
