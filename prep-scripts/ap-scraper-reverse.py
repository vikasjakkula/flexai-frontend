...
async def main():
    """Main scraper - REVERSE ORDER (processes tests from end to beginning)"""
    print("=" * 60)
    print("AP EAPCET Question Scraper - REVERSE ORDER MODE")
    print("=" * 60)
    print()
    print("CONFIGURATION:")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # REVERSE the pending tests list - process from end to beginning
            pending_tests.reverse()
            
            log(f"Pending tests: {len(pending_tests)}/{len(test_links)}")
            log(f"Processing in REVERSE ORDER (from end to beginning)")
            
            if not pending_tests:
                log("All tests already scraped!")
            else:
                # Scrape tests ONE BY ONE (sequential) in REVERSE ORDER
                for i, test_info in enumerate(pending_tests):
                    log(f"\n[{i+1}/{len(pending_tests)}] Starting next test (REVERSE MODE)...")
