"""
AP EAPCET Question Scraper - MIDDLE TO START VERSION
Scrapes questions, extracts sprite sheets + CSS, uploads to Supabase
No parallel execution - simple and reliable
Processes tests from MIDDLE to START (backwards from middle)
"""
            ...
            try:
                ...
                try:
                    ...
                    if count > 0:
                        ...
                        # Also check if we got meaningful content
...
async def main():
    """Main scraper - MIDDLE TO START (processes tests from middle backwards to start)"""
    print("=" * 60)
    print("AP EAPCET Question Scraper - MIDDLE TO START MODE")
    print("=" * 60)
    print()
    print("CONFIGURATION:")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Process from MIDDLE to START (backwards)
            middle_index = 0
            if pending_tests:
                middle_index = len(pending_tests) // 2
                pending_tests = pending_tests[:middle_index]  # Take from start to middle
                pending_tests.reverse()  # Reverse to go from middle backwards to start
            
            log(f"Pending tests: {len(pending_tests)}/{len(test_links)}")
            log(f"Processing from MIDDLE to START (backwards from index {middle_index})")
            
            if not pending_tests:
                log("All tests already scraped!")
            else:
                # Scrape tests ONE BY ONE (sequential) from MIDDLE to START
                for i, test_info in enumerate(pending_tests):
                    log(f"\n[{i+1}/{len(pending_tests)}] Starting next test (MIDDLE TO START MODE)...")
