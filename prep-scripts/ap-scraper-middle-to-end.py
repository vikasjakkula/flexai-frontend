...
async def main():
    """Main scraper - MIDDLE TO END (processes tests from middle to end)"""
    print("=" * 60)
    print("AP EAPCET Question Scraper - MIDDLE TO END MODE")
    print("=" * 60)
    print()
    print("CONFIGURATION:")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Process from MIDDLE to END
            middle_index = 0
            if pending_tests:
                middle_index = len(pending_tests) // 2
                pending_tests = pending_tests[middle_index:]  # Take from middle to end
            
            log(f"Pending tests: {len(pending_tests)}/{len(test_links)}")
            log(f"Processing from MIDDLE to END (starting from index {middle_index})")
            
            if not pending_tests:
                log("All tests already scraped!")
            else:
                # Scrape tests ONE BY ONE (sequential) from MIDDLE to END
                for i, test_info in enumerate(pending_tests):
                    log(f"\n[{i+1}/{len(pending_tests)}] Starting next test (MIDDLE TO END MODE)...")
