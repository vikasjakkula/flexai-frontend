/**
 * Categorize ALL uncategorized questions (~15k+) in one run.
 * Uses pagination to fetch every question (Supabase default limit is 1000).
 *
 * - Fetches all questions with chapter=null in chunks of 1000
 * - Processes everything with same logic as categorize-questions-chapters.js
 * - Resumable: run again to process any remaining (e.g. after failure)
 *
 * Run: node scripts/categorize-questions-chapters-full.js
...
async function fetchAllUncategorized(supabase, limit) {
  const all = [];
  let from = 0;
  let hasMore = true;
  let useChapterFilter = true;
  ...
  while (hasMore) {
    ...
    let query = supabase
      ...
      .range(from, to);
    if (useChapterFilter) query = query.is('chapter', null);

    const { data, error } = await query;

    if (error && error.code === '42703') {
      if (useChapterFilter) {
        console.log('Chapter column not found - fetching all questions.');
        useChapterFilter = false;
        all.length = 0;
        from = 0;
        continue;
      }
...
async function main() {
  ...
  console.log('Fetching ALL uncategorized questions (paginated, 1000 per request)...');
  const questions = await fetchAllUncategorized(supabase, opts.limit);
  const total = questions.length;

  const startTime = Date.now();

  console.log(`\n=== EAPCET Question Categorization (FULL) ===`);
